...
public class ArcaneJammer extends Spell {
	...
	public ArcaneJammer() {
		super(EnumTier.ADVANCED, 50, EnumElement.HEALING, "arcane_jammer", EnumSpellType.ATTACK, 100, EnumAction.none, false);
	}
	...
	public boolean cast(World world, EntityPlayer caster, int ticksInUse, float damageMultiplier, float rangeMultiplier, float durationMultiplier, float blastMultiplier) {
		...
		if(rayTrace != null && rayTrace.typeOfHit == MovingObjectType.ENTITY && rayTrace.entityHit instanceof EntityLivingBase){
			...
			if(!world.isRemote){
				entity.addPotionEffect(new PotionEffect(Wizardry.arcaneJammer.id, (int)(75*durationMultiplier), 0));
			}
		}
		if(world.isRemote){
