	...
	public boolean cast(World world, EntityPlayer caster, int ticksInUse, float damageMultiplier, float rangeMultiplier, float durationMultiplier, float blastMultiplier) {
		...
		if(rayTrace != null && rayTrace.typeOfHit == MovingObjectType.ENTITY && rayTrace.entityHit instanceof EntityLivingBase){
			...
			if(!world.isRemote){
				...
				EntityBubble entitybubble = new EntityBubble(world, entity.posX, entity.posY, entity.posZ, caster, (int)(100*durationMultiplier), false, damageMultiplier);
	...
	public boolean cast(World world, EntityLiving caster, EntityLivingBase target, float damageMultiplier, float rangeMultiplier, float durationMultiplier, float blastMultiplier){
		...
		if(target != null){
			...
			if(!world.isRemote){
				...
				// Deprecated in favour of entity riding method
				//entity.addPotionEffect(new PotionEffect(Wizardry.bubblePotion.id, 200, 0));
				EntityBubble entitybubble = new EntityBubble(world, target.posX, target.posY, target.posZ, caster, (int)(100*durationMultiplier), false, damageMultiplier);
				world.spawnEntityInWorld(entitybubble);
